return {
  ...
  config = function()
    ...
    vim.keymap.set("n", "<leader>fs", require("fzf-lua").live_grep_native, { desc = "[S]earch by [G]rep" })
    vim.keymap.set("n", "<leader>fr", require("fzf-lua").oldfiles, { desc = "Search [O]ld [F]iles" })
    ...
    vim.keymap.set("n", "<leader>fa", require("fzf-lua").git_branches, { desc = "[B]ranches" })
  end,
}
